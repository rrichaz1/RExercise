...
################################################################
# Drop claims_daysaway and testindex
################################################################
...
summary(trainDS)
#######################################
#Only tier has missing data - set missing values to 1
#######################################
...
###########################################################################################
##SAMPLING - As data is skewed towards outcome kust take 50-50 sample 
###############################################################
...
# Create the  test datasets for RFE prediction with the skewed data set
# dropped the NA here as the number of rows are enough
set.seed(100)
...
#Predict on test data taken from original data set
test.data.orig <-test.dataOrig.random
fitted <- predict(model.rf,test.data.orig)
Evaluate_F1_Score(fitted, as.factor(test.data.orig$outcome), "RFE Regression Test", 0.5)
EvaluateModel(fitted,  as.factor(test.data.orig$outcome), "RFE Regression Test", 0.5)
