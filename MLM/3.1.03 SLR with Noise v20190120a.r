...
m.slr.noi.theVpred <-
  lm(
    y1.slr.the.pred ~ y1.slr.noi.pred,
    data = df.slr.noi.theVpred
  )

summary(m.slr.noi.theVpred) # Perfect Fit: Slope = 1.016 and Intercept = -16. 
#                           # Represents relatively minor impact of noise over
#                           # such a broad range of x and y.
...
df.slr.noi.theVpred # See that each is off by a residual related to a slope not
#                   # equal to 1
