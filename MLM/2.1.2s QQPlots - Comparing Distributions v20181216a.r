...
# Scale x.norm and x.unif
x.norm.scale <- 
  (x.norm - mean(x.norm)) / sd(x.norm)
# It is this mean-centering, sd-normalizing that turns x.norm into a quantile
# for the Q-Q plot.  When x.norm is the x argument and no y argument is given
# the qqplot function automatically mean-centers and sd-normalizes. qqline() by
# contrast, in this context, mean-centers and sd-normalizes, hence the 
# "disappearance" of the qqline.
hist(
  x = x.norm.scale
  )
qqnorm(
...
# Q-Q Plot compare scaled x.norm and scaled x.unif
qqplot(
  x = x.norm.scale, 
  y = x.unif.scale
  )
...
lm( # Create a linear model object
  formula = x.unif ~ x.norm # Set x.unif as dependent and x.norm as independent
  )
...
# Sorting from smallest to largest is fundamental to quantile plotting
...
x.pp <- 
  ppoints(
    ...
    ) # Note that ppoints() automatically sorts smallest to largest
...
n1 <- 
  rnorm(
    n = 1000, 
    mean = 0, 
    sd = 1) # note already scaled because mean = 0 and sd = 1
qqnorm(
  y = n1
  )
t1 <- 
...
qqplot(
  x = n1, 
  y = t1
  ) # Curve is different because x-axis is different, it is derived from a
    # theoretical normal curve, rather than the observed n1
...
n3 <- 
  rnorm(
    ...
    mean = 10, 
    sd = 0.5
    ) # Note raw, unscaled because mean != 0 and sd <> 1
qqnorm(y = n3)
...
n4 <- 
  rnorm(
    ...
    mean = 150, 
    sd = 30
    ) # Note raw, unscaled and different
qqnorm(
  y = n4
  )
...
shapiro.test(x.norm) # FtRNH: data consistent with a Normal Distribution
shapiro.test(x.norm^4) # RNH: data inconsistent with a Normal Distribution
shapiro.test(x.norm.scale) # Identical to unscaled, as would be expected

shapiro.test(x.unif) # FtRNH: data consistent with a Normal Distribution
shapiro.test(x.unif^4)# RNH: data inconsistent with a Normal Distribution
shapiro.test(x.unif.scale) # Identical to unscaled, as would be expected

shapiro.test(x.chi) # RNH: data inconsistent with a Normal Distribution
shapiro.test(x.chi.scale) # Identical to unscaled, as would be expected
shapiro.test(x.chi.expect) # RNH: data inconsistent with a Normal Distribution

shapiro.test(n1) # FtRNH: data consistent with a Normal Distribution
shapiro.test(n2) # FtRNH: data consistent with a Normal Distribution

shapiro.test(t1) # RNH: data inconsistent with a Normal Distribution

shapiro.test(n3) # FtRNH: data consistent with a Normal Distribution
shapiro.test(n4) # FtRNH: data consistent with a Normal Distribution

shapiro.test(n5)  # Dance of the p-values!
...
x.uniform.obsvd <- 
  sort(
    ...
    ) #Narrow/Light tailed because tails drop to zero a x < 0 and x > 1
hist(
  x = x.uniform.obsvd, 
  prob = T
  )
...
qqplot(
  x = x.norm.expct, 
  y = x.cosine.obsvd, 
  main = 'Observed Platykurtic (Cosine)'
  )
abline(
  lm(
...
qqplot(
  x = x.norm.expct, 
  y = x.uniform.obsvd, 
  main = 'Observed Platykurtic (Uniform)'
  )
abline(
  lm(
...
qqplot(
  x = x.norm.expct, 
  y = x.beta.obsvd, 
  main = 'Observed Platykurtic (Beta)'
  )
abline(
  lm(
...
r <- 10 # Nbr of Trials, usually denoted with an N, but in OSDS it is an r
set.seed(207)
p <- ppoints(n)
xlim <- c(-10,10)
...
warnings() # Note because binom() expecting an integer input
